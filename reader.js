...
{
    let id = (await chrome.tabs.getCurrent()).id;
    chrome.runtime.sendMessage({id:id, type:"reader_lookup", x:x, y:y, text:text});
}

async function send_reader_mode(x, y, text)
{
    let id = (await chrome.tabs.getCurrent()).id;
    chrome.runtime.sendMessage({id:id, type:"reader_mode"});
}
...
{
    chrome.runtime.sendMessage({type:"gimmetext"});
}
