...
// --- Chrome MV3 port notes ---------------------------------------------
// This file runs as a Manifest V3 service worker, which has neither
// XMLHttpRequest nor DOM/Audio globals. The rest of this file is otherwise
// unmodified from the original Firefox background page, so:
//   1) We shim just enough of XMLHttpRequest (GET for local dict files, POST
//      for AnkiConnect) on top of fetch() so every existing load_* function
//      and the AnkiConnect mining code below works completely unchanged.
//   2) Anything that needs a real DOM (playing Audio, or the clipboard-paste
//      trick used by "gimmetext") is delegated to offscreen.html/offscreen.js
//      via chrome.offscreen + message passing, since service workers can't
//      touch the DOM at all. See ensure_offscreen_document() further down.
// -------------------------------------------------------------------------
class XMLHttpRequest
{
    ...
    overrideMimeType() {} // no-op; fetch negotiates content-type itself
    open(method, url)
    {
        this._method = method;
        this._url = url;
    ...
    send(body)
    {
        ...
        let timed_out = false;
        let timer = this.timeout ? setTimeout(() =>
        {
            timed_out = true;
            controller.abort();
        }, this.timeout) : null;
        ...
        }).then(async (res) =>
        {
            ...
            this.readyState = 4;
            for(let fn of (this._listeners["load"] || []))
                fn.call(this);
        }).catch((err) =>
        {
            if(timer) clearTimeout(timer);
            this.readyState = 4;
            if(timed_out)
                for(let fn of (this._listeners["timeout"] || []))
                    fn.call(this);
            else
                for(let fn of (this._listeners["error"] || []))
                    fn.call(this, err);
        });
    }
}

let offscreen_ready = null;
async function ensure_offscreen_document()
{
    ...
    if(!offscreen_ready)
    {
        offscreen_ready = chrome.offscreen.createDocument({
            url: "offscreen.html",
            reasons: ["AUDIO_PLAYBACK", "CLIPBOARD"],
            justification: "Play pronunciation audio clips and read clipboard text for the reader/clipboard-monitor feature"
        });
    }
    await offscreen_ready;
}
...
async function settings_init()
{
    try
    {
        ...
        {
            let temp = (await chrome.storage.local.get(name))[name];
            if(temp != undefined)
                settings[name] = temp;
        }
        getvar("reader_width");
...
chrome.storage.onChanged.addListener((updates, storageArea) =>
{
...
{
    return or_default((await chrome.storage.local.get(name))[name], fallback);
}
...
{
    let myjson = (await chrome.storage.local.get(["epwing"]))["epwing"];
    if(myjson !== undefined)
    {
        ...
        chrome.storage.local.set({"custom_dicts":custom_dicts});
        
        chrome.storage.local.remove(["epwing"]);
    }
...
function load_part_of_dictionary(filename)
{
    ...
    req.open("GET", chrome.runtime.getURL(filename));
    req.send();
    return req;
}
...
async function update_icon(enabled)
{
    if(enabled)
    {
        chrome.action.setTitle({title:"Nazeka (enabled)"});
        try
        {
    ...
    {
        chrome.action.setTitle({title:"Nazeka (disabled)"});
        try
        {
            chrome.action.setIcon({path:{
                "16": "img/action16.png",
...
{
    chrome.action.onClicked.addListener(toggle_enabled);
    fixicon();
}
...
function open_mining(info, tab)
{
    ...
    {
        chrome.windows.create({
            url:chrome.runtime.getURL("mining.html"),
            type:"popup"
        });
    } catch(err) {} // don't care
}
function open_livemining(info, tab)
{
    ...
    {
        chrome.windows.create({
            url:chrome.runtime.getURL("livemining.html"),
            type:"popup"
        });
    } catch(err) {} // don't care
}
function open_jsonconfig(info, tab)
{
    ...
    {
        chrome.windows.create({
            url:chrome.runtime.getURL("json_config.html"),
            type:"popup"
        });
    } catch(err) {} // don't care
}

// Chrome has never supported the Firefox-only `onclick` property on
// contextMenus.create() entries; every click has to be routed through a
// single chrome.contextMenus.onClicked listener keyed by menu item id.
const context_menu_handlers = {
    "nazeka-options": open_options,
    "nazeka-reader": open_reader,
    "nazeka-mining": open_mining,
    "nazeka-livemining": open_livemining,
    "nazeka-jsonconfig": open_jsonconfig,
    "nazeka-toggle": toggle_enabled,
};
...
// The original paste-target/execCommand("paste") trick needs a real DOM,
// which a Manifest V3 service worker doesn't have. That DOM now lives in
// offscreen.html/offscreen.js; we just ask it to grab the clipboard and
// wait for it to message the pasted text back to us.
async function clipboard_hook(tab)
{
    origin_tab = tab;
    await ensure_offscreen_document();
    chrome.runtime.sendMessage({type: "offscreen_do_paste"});
}
...
function send_error(tab, error)
{
    try
    {
        chrome.tabs.sendMessage(tab, {type:"error",error:error});
    }
    catch (err)
    {
...
chrome.runtime.onMessage.addListener((req, sender) =>
{
    ...
    else if (req.type == "offscreen_pasted_text")
    {
        // Relayed from offscreen.js after it ran the execCommand("paste")
        // trick on our behalf (service workers have no DOM to do this in).
        if(origin_tab != undefined)
            chrome.tabs.sendMessage(origin_tab.id, {type:"text", text:req.text});
    }
    ...
    {
        chrome.tabs.sendMessage(req.id, {type:req.type, x:req.x, y:req.y, text:req.text});
    }
    else if (req.type == "reader_mode")
    {
        chrome.tabs.sendMessage(req.id, {type:req.type});
    }
    ...
    {
        // A service worker has no `Audio` constructor; hand playback off to
        // the offscreen document, which is a real (if invisible) page.
        (async () =>
        {
            await ensure_offscreen_document();
            chrome.runtime.sendMessage({type: "offscreen_play_audio", host: req.host, volume: req.volume});
        })();
...
// Single dispatcher for every context menu item created above (Chrome has no
// per-item onclick callback the way Firefox does).
chrome.contextMenus.onClicked.addListener((info, tab) =>
{
    let handler = context_menu_handlers[info.menuItemId];
    if(handler)
        handler(info, tab);
});
