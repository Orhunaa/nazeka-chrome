...
async function get_real_platform()
{
    ...
    try {
        my_platform = await chrome.runtime.sendMessage({type:"platform"});
        if(my_platform)
...
async function buildpage()
{
    ...
    if(platform != "android")
    {
        ...
        liveminingbutton.onclick = function(e){chrome.windows.create({url:chrome.runtime.getURL('livemining.html'),type:'popup'}); e.preventDefault();};
        ...
        jsonmanagebutton.onclick = function(e){chrome.windows.create({url:chrome.runtime.getURL('json_config.html'),type:'popup'}); e.preventDefault();};
    ...
    else
    {
        ...
        jsonmanagebutton.type = "button";
        jsonmanagebutton.innerText = "Manage JSON Dictionaries";
        jsonmanagebutton.onclick = function(e){chrome.tabs.create({url:chrome.runtime.getURL('json_config.html')}); e.preventDefault();};
        optionsection.appendChild(jsonmanagebutton);
    ...
    decon_file.addEventListener("change", () =>
    {
        ...
        reader.onload = async(e) =>
        {
            try
            {
                ...
                if(text == "" || text == "\n" || text == "\r\n")
                    chrome.storage.local.set({"deconjugator_rules_json":""});
                else
                    chrome.storage.local.set({"deconjugator_rules_json":JSON.stringify(JSON.parse())});
    ...
    {
        let blob = new Blob([JSON.stringify((await chrome.storage.local.get()))], {type: 'text/plain'});
    ...
    backup_load.addEventListener("change", () =>
    {
        ...
        reader.onload = async(e) =>
        {
            try
            {
                ...
                chrome.runtime.sendMessage({type:"refreshjson"});
    ...
    abbrlink.onclick = function(e){chrome.windows.create({url:chrome.runtime.getURL('jmdictabbreviations.html'),type:'popup'}); e.preventDefault();};
}
...
async function restoreOptions()
{
    try
    {
        ...
        {
            let temp = (await chrome.storage.local.get(name))[name]; // FIXME do this for all options instead of once per option
            if(temp == undefined)
                temp = defval;
            return temp;
        }
...
function fixicon()
{
    chrome.runtime.sendMessage({type:"fixicon"});
}
...
function setOptions()
{
    ...
    chrome.storage.local.set(setstuff).then(()=>{fixicon();},()=>{});
}
