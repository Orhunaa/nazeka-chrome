...
function build_div_inner(text, result, moreText, index, first_of_many = false)
{
    ...
    if(settings.showoriginal)
    {
        ...
        if(first_of_many && (platform == "android" || is_sticky()))
        {
            ...
            left_arrow.src = chrome.runtime.getURL("img/leftarrow24.png");
            right_arrow.src = chrome.runtime.getURL("img/rightarrow24.png");
            ...
            if(platform == "android")
            {
                ...
                close_button.src = chrome.runtime.getURL("img/closebutton24.png");
...
{
    let cards = (await chrome.storage.local.get("cards")).cards;
    if(!cards)
        cards = [];
    cards.push(object);
...
async function try_to_play_audio(index = 0)
{
    ...
    if(fields)
    {
        ...
        chrome.runtime.sendMessage({type:"play_audio", host:url, volume:settings.volume});
    }
}
...
function send_ankiconnect_command(host, command)
{
    ...
    chrome.runtime.sendMessage({type:"ankiconnect_mine", host:host, command:json});
}
...
async function mine_to_ankiconnect_with_base64_audio(object, audiofname, audiodata)
{
    ...
    let promise = chrome.storage.local.get(["livemining_deckname", "livemining_modelname", "livemining_fields"]);
...
async function get_url_blob_base64(url)
{
    try
    {
        let data = await chrome.runtime.sendMessage({type:"get_audio_base64", url:url});
        return data;
    }
    catch(e)
    {
...
function keytest(event)
{
    ...
    if(event.key == settings.hotkey_kanji_mode)
    {
        ...
        chrome.storage.local.set({"kanji_mode":settings.kanji_mode});
        if(exists_div())
    ...
    if(event.key == settings.hotkey_sticky)
    {
        ...
        chrome.storage.local.set({"sticky":settings.sticky});
        if(exists_div())
        {
